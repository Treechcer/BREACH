    ...
    for _, val in ipairs(vertices.objs) do
        ...
        for index, value in ipairs(val.lineWire) do
            for i=1, #value do
                ...
                local Apos = vectors.add(vectors.rotate(a, {yaw = val.angle.yaw + angle.yaw, pitch = val.angle.pitch + angle.pitch}), {x = val.move.x + mv.x, y = val.move.y + mv.y, z = val.move.z + mv.z})
                local Bpos = vectors.add(vectors.rotate(b, {yaw = val.angle.yaw + angle.yaw, pitch = val.angle.pitch + angle.pitch}), {x = val.move.x + mv.x, y = val.move.y + mv.y, z = val.move.z + mv.z})
                if Apos.z > 0 and Bpos.z > 0 then
                    renderer.line(vectors.screenCordinates(vectors.project(Apos)), vectors.screenCordinates(vectors.project(Bpos)))
                end
            end
        end
    end
